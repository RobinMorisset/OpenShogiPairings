...
Mutations go through a `TournamentStore` that keeps the current tournament plus
two stacks of snapshots: the undo history, and the changes undone since the last
mutation. Undo moves an entry from the first to the second, redo moves it back,
and any new mutation discards the second — history is a line, not a tree.
Create/load/restore reset both, which also stops a redo from splicing in a
snapshot carrying a different tournament's id. Each entry carries a
`ChangeLabel` naming the change it sits beside — a stable machine code plus
interpolation values, like an error's (`crates/server/src/history.rs`) — because
a snapshot alone cannot say what an undo would take back, and the button reverts
whichever referee acted last. Endpoints return a `TournamentView` — the
tournament, `undo_label` and `redo_label` (absent when there is nothing that
way, which is also what disables the button), the change `version`,
server-computed `standings`, the cup podium (once decided), and suggested
handicaps per board — so the client refreshes the view, both history buttons,
and the ranked table together from one response (the persisted save-file shape
stays the bare `Tournament`). `standings` is computed
